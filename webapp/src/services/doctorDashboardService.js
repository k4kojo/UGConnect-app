...
class DoctorDashboardService {
  ...
  async createDirectPrescription(prescriptionData) {
    try {
      const response = await prescriptionsAPI.createDirectPrescription(prescriptionData);
      return {
        success: true,
  ...
  async createAppointmentPrescription(prescriptionData) {
    try {
      const response = await prescriptionsAPI.createAppointmentPrescription(prescriptionData);
      return {
        success: true,
        data: response.data || response
      };
  ...
  async getNotifications() {
    try {
      console.log('DoctorDashboardService: Fetching notifications from API...');
      const startTime = Date.now();
      const response = await notificationsAPI.getUser();
      const endTime = Date.now();
      console.log(`DoctorDashboardService: Notifications fetch completed in ${endTime - startTime}ms, received ${response.data?.length || 0} notifications`);
      ...
      console.error('DoctorDashboardService: Error fetching notifications:', {
