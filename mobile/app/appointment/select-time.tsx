...
export default function SelectTimeScreen() {
  ...
  const { initials, name, specialty, doctorId, fee } = useLocalSearchParams();

  const handleContinue = () => {
    if (!selectedDate || !selectedTime) {
      alert(t("selectTime.selectDateTimeFirst"));
      return;
    }
    ...
    >
      ...
      <View style={styles.headerRow}>
        <TouchableOpacity style={styles.backBtn} onPress={() => router.back()}>
          ...
          <Text style={{ color: themeColors.text, fontSize: 18 }}>{t("common.back")}</Text>
        </TouchableOpacity>
      </View>
      ...
      >
        <Avatar fullName={typeof name === "string" ? name : undefined} size={46} border containerStyle={{ backgroundColor: Colors.brand.avatarBg, borderColor: Colors.brand.avatarBg }} />
        <View>
          <Text style={[styles.doctorName, { color: themeColors.text }]}>
            {name ?? t("confirm.doctorNameFallback")}
          </Text>
          <Text style={[styles.specialty, { color: themeColors.subText }]}>
            {specialty ?? t("confirm.specialtyFallback")}
          </Text>
        </View>
      </View>
        ...
        {["Video", "In-Person"].map((type) => {
          ...
          return (
            ...
            >
              ...
              >
                {type === "Video" ? t("appointments.videoCall") : t("confirm.inPersonVisit")}
              </Text>
            </TouchableOpacity>
          );
        })}
