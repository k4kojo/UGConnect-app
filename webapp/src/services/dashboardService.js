...
class DashboardService {
    ...
    try {
      ...
      const enhancedLabResults = labResults.map(result => {
        ...
        return {
          ...
          orderedByName: orderedBy ? `${orderedBy.firstName} ${orderedBy.lastName}` : 'Unknown Staff',
  ...
  async getNotifications(userRole = 'admin') {
    try {
      ...
      console.log(`DashboardService: Notifications fetch completed in ${endTime - startTime}ms, received ${response.data?.length || 0} notifications`);
  ...
  async getAppointments() {
    try {
      ...
      const endTime = Date.now();
      console.log(`DashboardService: Appointments fetch completed in ${endTime - startTime}ms, received ${response.data?.length || 0} appointments`);
  ...
  async generateReport(reportType, params = {}) {
    try {
      ...
      const reportData = {
        ...
        type: reportType,
        content: params.content || `This is a ${reportType} report generated with the following parameters: ${JSON.stringify(params, null, 2)}`
      };
  ...
  async createReport(reportData) {
    try {
      ...
      return {
        success: false,
        error: error.response?.data?.error || error.message || 'Failed to create report'
      };
    }
  }
  ...
  async updateReport(reportId, reportData) {
    try {
      ...
      return {
        success: false,
        error: error.response?.data?.error || error.message || 'Failed to update report'
      };
    }
  }
  ...
  async updateDoctorAvailability(availabilityId, availabilityData) {
    try {
      ...
      if (availabilityId && availabilityId !== null) {
        ...
        response = await availabilityAPI.update(availabilityId, availabilityData);
      } else {
